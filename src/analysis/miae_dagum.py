...
# Create empty list for ISE, and dictionaries for MIAE and the optimal
# bandwidth.
ise = []
miae = {}
        ...
        for d in range(draws['configuration_0']):
            ...
            def integrand(x):
                return (np.absolute(kde._density_value(x, bandwidth) - (
                    densities['density_0']['alpha'] *
                    burr.pdf(x,
                        densities['density_0']['c_1'],
                        densities['density_0']['d_1']) +
                    densities['density_0']['beta'] *
                    burr.pdf(x,
                        densities['density_0']['c_2'],
                        densities['density_0']['d_2'])
                )))
...
with open(ppj('OUT_ANALYSIS', 'miae_dagum.pickle'), 'wb') as f_results:
